...
import {
  ArrowUp, CheckCircle, ChevronDown, Clock, Filter, MessageCircle,
...
export default function DiscussionsPage() {
  ...
  return (
    <div className="bg-neutral-100 min-h-screen">
      <section className="bg-gradient-to-br from-violet-600 via-purple-500 to-primary-600 text-white border-b-4 border-secondary-500">
            ...
            {showNew && (
              <Reveal>
                <div className="card p-5 border-2 border-violet-200">
                  ...
                  <input type="text" placeholder="Discussion title..." className="input-field mb-3" />
                  <div className="grid sm:grid-cols-2 gap-3 mb-3">
                    <select className="select-field text-sm">{categories.filter(c => c !== "All").map(c => <option key={c}>{c}</option>)}</select>
                    <input type="text" placeholder="Tags (comma-separated)" className="input-field text-sm" />
                  </div>
                  <textarea placeholder="Write your discussion..." className="input-field h-24 resize-none mb-3" />
                  <div className="flex gap-2"><button className="btn-primary text-sm">Post Discussion</button><button onClick={() => setShowNew(false)} className="text-sm text-neutral-500 hover:text-neutral-700">Cancel</button></div>
                </div>
              </Reveal>
            )}
