...
const INITIAL_FEED: FeedPost[] = [
  {
    id: 1, author: 'Maria G.', avatar: 'MG', club: 'TSA', time: '2 hours ago',
    ...
    type: 'resource', fileName: 'TSA_Presentation_Templates.zip', fileSize: '4.2 MB',
    likes: 24, liked: false, saved: false,
    replies: [
      { id: 1, author: 'James L.', avatar: 'JL', text: 'This is incredible, thanks Maria! Using this for regionals.', time: '1h ago' },
      { id: 2, author: 'Sophie K.', avatar: 'SK', text: 'The slide layouts are really clean. Great work!', time: '45m ago' },
    ],
  },
  {
    id: 2, author: 'Alex J.', avatar: 'AJ', club: 'Robotics', time: '5 hours ago',
    text: 'Does anyone have experience with PID tuning for FTC robots? We keep overshooting our target position.',
    type: 'discussion', likes: 8, liked: false, saved: false,
    replies: [
  ...
  {
    id: 3, author: 'Sophie K.', avatar: 'SK', club: 'NHS', time: '1 day ago',
    ...
    type: 'achievement', likes: 42, liked: false, saved: false, replies: [],
  },
  {
    id: 4, author: 'James L.', avatar: 'JL', club: 'FBLA', time: '1 day ago',
    ...
    type: 'resource', fileName: 'FBLA_Fundraiser_Tracker.xlsx', fileSize: '1.8 MB',
    likes: 31, liked: false, saved: false,
    replies: [
      { id: 1, author: 'Maria G.', avatar: 'MG', text: 'The formulas in this are next level. Sharing with our treasurer!', time: '22h ago' },
    ],
  },
  {
    id: 5, author: 'Taylor M.', avatar: 'TM', club: 'Drama', time: '2 days ago',
    text: 'Spring musical auditions are next week! Drop your best monologue tips below.',
    type: 'text', likes: 15, liked: false, saved: false, replies: [],
  },
  {
    id: 6, author: 'Priya K.', avatar: 'PK', club: 'Debate', time: '3 days ago',
    text: 'Uploading my evidence files and case briefs from last tournament. Hope these help someone prepping for districts!',
    type: 'resource', fileName: 'Debate_Evidence_Briefs.pdf', fileSize: '2.5 MB',
    likes: 19, liked: false, saved: false,
    replies: [
      { id: 1, author: 'Alex J.', avatar: 'AJ', text: 'These sources are gold. Thank you so much!', time: '2d ago' },
    ],
  },
];

const MENTORS = [
  { name: 'Greg Shelton', avatar: 'GS', role: 'Webmaster Advisor', specialty: 'Web Development & TSA', available: true },
  { name: 'Dr. Patel', avatar: 'DP', role: 'STEM Coordinator', specialty: 'Robotics & Engineering', available: true },
  { name: 'Ms. Rivera', avatar: 'MR', role: 'FBLA Advisor', specialty: 'Business & Leadership', available: false },
];

const MEETINGS = [
  { id: 1, title: 'TSA Chapter Weekly Sync', club: 'TSA', time: 'Today, 4:00 PM', attendees: 12, live: true },
  { id: 2, title: 'Robotics Build Session', club: 'Robotics', time: 'Today, 5:30 PM', attendees: 8, live: false },
  { id: 3, title: 'NHS Volunteer Planning', club: 'NHS', time: 'Tomorrow, 3:00 PM', attendees: 15, live: false },
  { id: 4, title: 'FBLA Competition Prep', club: 'FBLA', time: 'Wed, 4:00 PM', attendees: 6, live: false },
];
...
export default function CommunityPage() {
  ...
  function toggleReplies(id: number) {
    setExpandedReplies(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; });
  }

  function submitReply(postId: number) {
    const text = replyInputs[postId]?.trim();
    if (!text) return;
    setFeed(prev => prev.map(p => p.id === postId ? { ...p, replies: [{ id: Date.now(), author: userName || 'You', avatar: userInitials, text, time: 'Just now' }, ...p.replies] } : p));
          ...
          <div className="hidden lg:block w-56 shrink-0 space-y-4">
            ...
            <div className="bg-white border border-neutral-200 overflow-hidden">
              ...
              <div className="px-4 pb-4 -mt-5">
                {avatarUrl ? (
                  <Image src={avatarUrl} alt={userName} width={48} height={48} className="w-12 h-12 border-2 border-white object-cover" />
                ) : (
                  <div className="w-12 h-12 bg-primary-100 border-2 border-white flex items-center justify-center text-sm font-bold text-primary-700">{userInitials}</div>
                )}
                <h3 className="font-bold text-sm text-primary-700 mt-2">{userName || 'Your Name'}</h3>
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            >
              ...
              <div className="flex items-center gap-3 px-5 pt-4 pb-2">
                {avatarUrl ? (
                  <Image src={avatarUrl} alt={userName} width={44} height={44} className="w-11 h-11 object-cover shrink-0" />
                ) : (
                  <div className="w-11 h-11 bg-primary-600 text-white flex items-center justify-center text-sm font-bold shrink-0">{userInitials}</div>
                )}
                <textarea
                  value={postText}
              ...
              {attachedFile && (
                <div className="mx-5 mb-2 flex items-center gap-3 bg-primary-50 border border-primary-200 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-primary-500">{(attachedFile.size / 1024 / 1024).toFixed(2)} MB &middot; Ready to upload</p>
                  </div>
                  <button onClick={() => { setAttachedFile(null); if (fileInputRef.current) fileInputRef.current.value = ''; }} className="text-neutral-400 hover:text-neutral-600 p-1">
                    <X size={16} />
                  </button>
                </div>
              )}
              ...
              {!attachedFile && (
                <div className="mx-5 mb-3 border-2 border-dashed border-neutral-300 bg-neutral-50 py-5 flex flex-col items-center gap-2 cursor-pointer hover:border-primary-400 hover:bg-primary-50/30 transition-colors" onClick={() => fileInputRef.current?.click()}>
                  <Upload size={28} className="text-neutral-400" />
                  <p className="text-sm font-semibold text-neutral-600">Drop files here or click to browse</p>
                  <p className="text-[11px] text-neutral-400">PDF, DOCX, PPTX, XLSX, ZIP, Images &middot; Max 25MB</p>
                </div>
              )}
              ...
              <div className="flex items-center justify-between px-5 py-3 border-t border-neutral-100">
                <div className="flex gap-1.5">
                  <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold bg-primary-50 border border-primary-200 text-primary-700 hover:bg-primary-100 transition-colors">
                    <Upload size={14} /> Upload
                  </button>
                  <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold bg-neutral-50 border border-neutral-200 text-neutral-600 hover:bg-neutral-100 transition-colors">
            ...
            {filtered.slice(0, 4).map(post => (
              <div key={post.id} className="bg-white border border-neutral-200">
                {/* Header */}
                <div className="flex items-center gap-3 px-5 pt-4 pb-2">
                  <div className="w-10 h-10 bg-primary-100 text-primary-700 flex items-center justify-center text-xs font-bold shrink-0">{post.avatar}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-primary-700">{post.author}</span>
                      <span className={`text-[10px] px-1.5 py-0.5 font-semibold ${typeColors[post.type]}`}>{post.club}</span>
                    </div>
                ...
                <div className="flex items-center border-t border-neutral-100 px-2">
                  <button onClick={() => toggleLike(post.id)} className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold transition-colors ${post.liked ? 'text-accent-600' : 'text-neutral-400 hover:text-accent-500'}`}>
                    {post.liked ? <Heart size={16} fill="currentColor" /> : <Heart size={16} />}
                  ...
                  <button onClick={() => toggleReplies(post.id)} className="flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold text-neutral-400 hover:text-primary-600 transition-colors">
                    <MessageCircle size={16} /> {post.replies.length} {post.replies.length === 1 ? 'Reply' : 'Replies'}
                  </button>
                  <button onClick={() => toggleSave(post.id)} className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold transition-colors ${post.saved ? 'text-secondary-600' : 'text-neutral-400 hover:text-secondary-500'}`}>
                    {post.saved ? <Bookmark size={16} fill="currentColor" /> : <Bookmark size={16} />} Save
                  </button>
                ...
                {expandedReplies.has(post.id) && (
                  <div className="border-t border-neutral-100 bg-neutral-50/70">
                    ...
                    <div className="flex items-center gap-3 px-5 py-3 border-b border-neutral-100">
                      {avatarUrl ? (
                        <Image src={avatarUrl} alt={userName} width={32} height={32} className="w-8 h-8 object-cover shrink-0" />
                      ) : (
                        <div className="w-8 h-8 bg-primary-600 text-white flex items-center justify-center text-[10px] font-bold shrink-0">{userInitials}</div>
                      )}
                      <input
                      ...
                      />
                      <button onClick={() => submitReply(post.id)} className="px-3 py-2 bg-primary-600 text-white hover:bg-primary-700 transition-colors">
                        <Send size={14} />
                      </button>
                    </div>
                    ...
                    <div className="max-h-[350px] overflow-y-auto">
                      {post.replies.map(r => (
                        <div key={r.id} className="flex gap-3 px-5 py-3 border-b border-neutral-100 last:border-b-0">
                          <div className="w-8 h-8 bg-primary-100 text-primary-700 flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5">{r.avatar}</div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
          ...
          <div className="hidden xl:block w-72 shrink-0 space-y-4">
            ...
            <div className="bg-white border border-neutral-200">
              <div className="px-4 py-3 border-b border-neutral-200 flex items-center justify-between">
                <h3 className="text-sm font-bold text-primary-700 flex items-center gap-2"><GraduationCap size={15} /> Mentor Hub</h3>
                <Link href="/hub/mentors" className="text-[10px] font-semibold text-secondary-600 hover:text-secondary-700">See All</Link>
              </div>
              <div className="divide-y divide-neutral-100">
                {MENTORS.map((m, i) => (
                  <div key={i} className="px-4 py-3 flex items-start gap-3">
                    <div className="relative shrink-0">
                      <div className="w-9 h-9 bg-primary-100 text-primary-700 flex items-center justify-center text-xs font-bold">{m.avatar}</div>
                      {m.available && <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-green-500 border-2 border-white rounded-full" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-primary-700">{m.name}</p>
                      <p className="text-[11px] text-neutral-500">{m.role}</p>
                      <p className="text-[10px] text-neutral-400">{m.specialty}</p>
                    </div>
                    <Link href="/hub/mentors" className="px-2 py-1 text-[10px] font-semibold bg-primary-50 border border-primary-200 text-primary-700 hover:bg-primary-100 transition-colors shrink-0">
                      Connect
                    </Link>
                  </div>
                ))}
            ...
            <div className="bg-white border border-neutral-200">
              <div className="px-4 py-3 border-b border-neutral-200 flex items-center justify-between">
                <h3 className="text-sm font-bold text-primary-700 flex items-center gap-2"><Video size={15} /> Meetings</h3>
                <Link href="/meetings" className="text-[10px] font-semibold text-secondary-600 hover:text-secondary-700">All</Link>
              </div>
              <div className="divide-y divide-neutral-100">
                {MEETINGS.map(mt => (
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          {mt.live && <span className="w-2 h-2 bg-red-500 rounded-full animate-pulse shrink-0" />}
                          <span className="text-[10px] font-semibold px-1.5 py-0.5 bg-primary-50 text-primary-700 border border-primary-100">{mt.club}</span>
                        </div>
                        <p className="text-sm font-semibold text-primary-700 truncate">{mt.title}</p>
                        <div className="flex items-center gap-2 mt-1 text-[11px] text-neutral-400">
                          <span className="flex items-center gap-1"><Clock size={11} /> {mt.time}</span>
            ...
            <div className="bg-white border border-neutral-200">
              <div className="px-4 py-2.5 border-b border-neutral-200 flex items-center justify-between">
                <h3 className="text-xs font-bold text-primary-700 flex items-center gap-1.5"><MessageCircle size={13} /> Messages</h3>
                <span className="w-2 h-2 bg-green-500 rounded-full" />
              </div>
              <div className="h-44 overflow-y-auto p-3 space-y-2">
                {chatMessages.map((msg, i) => (
                  <div key={i} className={`px-3 py-2 text-xs ${msg.user === 'You' ? 'bg-primary-50 border-l-2 border-primary-400' : 'bg-neutral-50'}`}>
                    <div className="flex items-center gap-1.5 mb-0.5">
              ...
              <div className="flex border-t border-neutral-200">
                ...
                <button onClick={sendChat} className="px-3 text-primary-600 hover:text-primary-700"><Send size={14} /></button>
              </div>
            </div>
          </div>
        </div>
