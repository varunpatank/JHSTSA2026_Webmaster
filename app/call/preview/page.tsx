...
import { Mic, MicOff, Video, VideoOff, Monitor, PhoneCall, ArrowLeft, Users, Copy, Check } from "lucide-react";
import Whiteboard from "@/components/Whiteboard";

function CallPreviewInner() {
  const searchParams = useSearchParams();
  const roomParam = searchParams.get("room");
  const room = roomParam || `ClubConnect-${Date.now()}`;
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="bg-neutral-50  p-3">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={copyLink} className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-primary-100 text-primary-700  hover:bg-primary-200 transition-colors">
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col">
      <div className="flex items-center justify-between px-4 py-3 bg-white/95 shadow-md">
        ...
        <div className="flex items-center gap-2">
          <button onClick={copyLink} className="px-3 py-1.5 text-sm border border-neutral-200  hover:bg-neutral-50 flex items-center gap-1.5 transition-colors">
            {copied ? <Check size={14} /> : <Copy size={14} />}
            {copied ? "Copied!" : "Invite"}
          </button>
          <button onClick={() => setMicOn(!micOn)} className={`w-9 h-9  flex items-center justify-center transition-all ${micOn ? "bg-primary-500 text-white" : "bg-red-500 text-white"}`}>
            {micOn ? <Mic size={16} /> : <MicOff size={16} />}
          </button>
          <button onClick={() => setCamOn(!camOn)} className={`w-9 h-9  flex items-center justify-center transition-all ${camOn ? "bg-primary-500 text-white" : "bg-red-500 text-white"}`}>
            {camOn ? <Video size={16} /> : <VideoOff size={16} />}
          </button>
          <button onClick={() => setShowWhiteboard(!showWhiteboard)} className={`px-3 py-1.5 text-sm  border transition-all flex items-center gap-1.5 ${showWhiteboard ? "bg-primary-500 text-white border-primary-500" : "border-neutral-200 text-neutral-600 hover:bg-neutral-50"}`}>
            <Monitor size={14} /> Whiteboard
          </button>
          <Link href="/meetings" className="px-3 py-1.5 bg-red-500 text-white text-sm  hover:bg-red-600 transition-colors">
            Leave
          </Link>
        </div>
      </div>
        ...
        <div className={`flex-1 p-4 ${showWhiteboard ? "w-1/2" : "w-full"}`}>
          <div className="h-full bg-gradient-to-br from-slate-800 to-slate-900  flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <div className="relative z-10 text-center">
              ...
              <p className="text-white/40 text-sm mt-6">Share the meeting link to invite others to this room</p>
            </div>
          </div>
        </div>
