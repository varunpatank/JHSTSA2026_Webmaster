...
export default function AlumniPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "network" | "mentorship" | "careers" | "support"
  >("network");
  const [actionMessage, setActionMessage] = useState<string>("");

  const sendAlumAction = (
    subject: string,
    body: string,
    navigateTo?: string,
  ) => {
    setActionMessage(
      "Action complete. You will receive an email from ALUM with next steps.",
    );
    window.open(
      `mailto:alum@clubconnect.school?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`,
      "_blank",
    );
    if (navigateTo) {
  ...
  return (
    <div className="bg-neutral-100 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        {actionMessage && (
          <div className="mb-4 border border-green-200 bg-green-50 text-green-700 text-sm font-semibold px-4 py-2">
            {actionMessage}
          </div>
        )}
        {activeTab === "network" && (
          <>
            <div className="card p-4 mb-6">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search alumni by name, college, club, or career..."
                  value={search}
        ...
        {activeTab === "mentorship" && (
          <div className="space-y-6">
                  ...
                  {[
                    {
                      ...
                      desc: "Find alumni with expertise in your interests or career goals.",
                    },
                    {
                      step: "2",
                      title: "Request Connection",
                      desc: "Send a mentorship request with your goals and preferred schedule.",
                    },
                    {
                      step: "3",
                      title: "Meet & Grow",
                      desc: "Have regular check-ins, get advice, and build your professional network.",
                    },
                  ].map((s) => (
                    <div
                      key={s.step}
                      className="bg-white  p-4 border border-neutral-200 text-center"
                    >
                      <div className="w-10 h-10 rounded-full bg-primary-500 text-white flex items-center justify-center mx-auto text-sm font-bold">
                        {s.step}
                      </div>
                      <h3 className="font-bold text-primary-700 mt-2">
                        {s.title}
                      </h3>
            ...
            <Reveal>
              <h2 className="text-lg font-heading font-bold text-primary-600 mb-3">
                Available Mentors
              </h2>
                ...
                {featuredAlumni
                  ...
                  .map((a) => (
                    <div key={a.id} className="card p-5 ux-hover-lift-sm">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12  bg-secondary-50 text-secondary-700 flex items-center justify-center font-bold shrink-0">
                          {a.name
                            .split(" ")
                            .map((n) => n[0])
                            .join("")}
                      ...
                      <button
                        className="btn-outline text-sm w-full mt-4"
                        onClick={() =>
                          sendAlumAction(
                            `Mentorship request: ${a.name}`,
                            `Please connect me with ${a.name} (${a.career}).`,
                            "/hub/mentors",
                          )
                        }
                      >
                  ...
                  <div key={panel.id} className="card p-5 ux-hover-lift-sm">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      ...
                      <button
                        ...
                        onClick={() =>
                          sendAlumAction(
                            `Career panel registration: ${panel.title}`,
                            `Please register me for ${panel.title} on ${new Date(panel.date).toLocaleDateString()} at ${panel.time}.`,
                          )
                        }
                      >
                        Register ({panel.registrations} registered)
                      </button>
                    </div>
                  </div>
                ))}
              ...
              <div className="card p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    <div
                      key={field}
                      className="bg-primary-50 border border-primary-100  p-3 text-center text-sm font-semibold text-primary-700 ux-hover-lift-sm"
                    >
                      {field}
                    </div>
                  ))}
            ...
            <Reveal>
              <div className="card p-6 bg-gradient-to-r from-red-50/50 to-secondary-50/50">
                <h2 className="text-xl font-heading font-bold text-primary-600 flex items-center gap-2">
                  <Heart size={18} /> Support Current Chapters
                </h2>
                <p className="text-sm text-neutral-600 mt-2">
                  Alumni can support current clubs through donations, guest
                  speaking, mentorship, or sponsoring events.
                </p>
                <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  {[
                    {
                      title: "Donate",
                      desc: "Fund supplies, trips, and competitions",
                      link: "/donate",
                      subject: "Alumni donation interest",
                    },
                    {
                      title: "Guest Speak",
                      desc: "Share your career journey with students",
                      link: "/about",
                      subject: "Alumni guest speaker interest",
                    },
                    {
                      title: "Mentor",
                      desc: "Guide the next generation of leaders",
                      link: "/hub/mentors",
                      subject: "Alumni mentor signup",
                    },
                    {
                      title: "Sponsor Events",
                      desc: "Support fundraisers and activities",
                      link: "/events",
                      subject: "Alumni event sponsorship",
                    },
                  ].map((a) => (
                    <button
                      key={a.title}
                      ...
                      className="bg-white  p-4 border border-neutral-200 hover:border-primary-300 ux-hover-lift-sm text-center"
                    >
              ...
              <div className="card p-6">
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  {[
                    ...
                    <div
                      key={s.label}
                      className={`${s.color}  p-5 text-center border ux-hover-lift-sm`}
                    >
