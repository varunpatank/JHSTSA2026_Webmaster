...
function DirectoryPageContent() {
  ...
  useEffect(() => {
    ...
    organizationsApi.getAll().then(({ data }) => {
      if (!data) return;
      const orgs = (data as Organization[]).map(
        (org): Chapter => ({
          id: org.id,
          name: org.name,
          description: org.description || "",
          category: (org.category || "General") as Chapter["category"],
          meetingFrequency: (org.meeting_frequency ||
            "Weekly") as Chapter["meetingFrequency"],
          membershipStatus: (org.membership_status ||
            "Open Enrollment") as Chapter["membershipStatus"],
          gradeLevel: (org.grade_level ||
            "All Grades") as Chapter["gradeLevel"],
          meetingTime: (org.meeting_time ||
            "After School") as Chapter["meetingTime"],
          advisor: {
            name: org.advisor_name || "",
          ...
          meetingLocation: {
            lat: 47.605,
            lng: -122.3344,
            parentOrg: "Seattle High School",
            room: org.meeting_location || "",
          },
      ...
      setDbChapters(orgs.filter((c) => c.name !== "Other Clubs"));
    });
  }, []);
  const chapters = useMemo(() => {
    ...
    const unique = created.filter(
      (c) => !staticIds.has(c.id) && !deletedOrgIds.has(c.id),
    );
    const allIds = new Set([
      ...staticChapters.map((c) => c.id),
      ...unique.map((c) => c.id),
    ]);
    const fromDb = dbChapters.filter(
      (c) => !allIds.has(c.id) && !deletedOrgIds.has(c.id),
    );
  ...
  return (
    <div className="relative">
      <div
        ...
        style={{
          backgroundImage:
            "repeating-linear-gradient(45deg, transparent, transparent 18px, rgba(30,58,95,0.08) 18px, rgba(30,58,95,0.08) 19px)",
        }}
      />
      <div className="relative z-0 bg-cream-200 min-h-screen">
        <HeroSection
          ...
          description={
            <>
              Browse{" "}
              <strong className="text-secondary-700 font-bold">
                active clubs
              </strong>{" "}
              across STEM, arts, service, leadership, cultural, and more. Filter
              by interest area, meeting schedule, or open enrollment status,
              then{" "}
              <strong className="text-primary-700 font-semibold">
                connect directly with club officers
              </strong>{" "}
          ...
          actions={
            <Link
              href="/portal"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-secondary-500 hover:bg-secondary-600 text-white text-sm font-bold transition-colors"
            >
                    ...
                    <div className="p-3.5 flex flex-col gap-3.5 overflow-hidden flex-1 bg-gradient-to-b from-white to-cream-50">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          ...
                          {
                            value: new Set(chapters.map((c) => c.category))
                              .size,
                            label: "Categories",
                          },
                        ].map((stat) => (
                      ...
                      <div className="bg-white border border-neutral-200 overflow-hidden flex-1 flex flex-col min-h-0">
                        <div className="bg-secondary-500/10 border-b border-neutral-200 px-3.5 py-2.5 flex items-center gap-2">
                          ...
                          <h3 className="text-xs font-heading font-bold uppercase tracking-wider text-primary-900">
                            Most Popular
                          </h3>
                        </div>
                        <div className="divide-y divide-neutral-100 overflow-y-auto flex-1">
                          {[...chapters]
                            ...
                            .map((club, i) => (
                              ...
                              >
                                <span className="text-[10px] font-bold text-neutral-400 w-4 shrink-0">
                                  {i + 1}
                                </span>
                                <div className="min-w-0 flex-1">
                                  <p className="text-xs font-semibold text-primary-900 truncate group-hover:text-primary-700">
                                    {club.name}
                                  </p>
                                ...
                                <ArrowRight
                                  size={11}
                                  className="text-neutral-300 group-hover:text-primary-400 shrink-0"
                                />
                              </Link>
                            ))}
                        </div>
