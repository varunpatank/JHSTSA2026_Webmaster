-- ═══════════════════════════════════════════════════════════════════════
-- Migration 007: Complete Schema
-- Consolidates all gaps, adds missing tables, ensures full coverage
-- for: club creation, event creation, resource uploads, comments,
-- statistics, ratings, bookmarks, advisors, and more.
--
...
-- ═══════════════════════════════════════════════
-- 2. CORE TABLES (idempotent creates + column adds)
-- ═══════════════════════════════════════════════
...
ALTER TABLE resources ADD COLUMN IF NOT EXISTS category varchar(50);
...
ALTER TABLE resources ADD COLUMN IF NOT EXISTS file_size varchar(50);
ALTER TABLE resources ADD COLUMN IF NOT EXISTS format varchar(20);
...
-- Comments (polymorphic: org, event, resource, or discussion)
...
-- Club Proposals (Start-a-Club wizard submissions)
...
ALTER TABLE club_proposals ADD COLUMN IF NOT EXISTS logo_url text;
ALTER TABLE club_proposals ADD COLUMN IF NOT EXISTS poster_url text;

-- Advisors (faculty advisors linked to clubs)
...
ALTER TABLE org_analytics ADD COLUMN IF NOT EXISTS meetings_held integer DEFAULT 0;
ALTER TABLE org_analytics ADD COLUMN IF NOT EXISTS service_hours_total numeric(7,2) DEFAULT 0;
ALTER TABLE org_analytics ADD COLUMN IF NOT EXISTS competition_wins integer DEFAULT 0;
...
ALTER TABLE donations ADD COLUMN IF NOT EXISTS donor_id uuid REFERENCES profiles(id) ON DELETE SET NULL;
...
ALTER TABLE uploads ADD COLUMN IF NOT EXISTS likes integer DEFAULT 0;

-- ═══════════════════════════════════════════════
-- 11. NEW TABLES: RATINGS, BOOKMARKS, ACTIVITY LOG
-- ═══════════════════════════════════════════════

-- Ratings & Reviews (for clubs)
...
-- Bookmarks / Saved Items (polymorphic)
...
-- Activity Log (tracks user actions for feeds)
...
-- Upload Likes (track who liked community uploads)
...
-- ═══════════════════════════════════════════════
-- 14. HELPER FUNCTIONS (idempotent)
-- ═══════════════════════════════════════════════

-- Org admin check (SECURITY DEFINER to avoid recursive RLS)
...
-- Auto-update member_count on membership changes
...
-- Auto-update event attendee count on registration changes
